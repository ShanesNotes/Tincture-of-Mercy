...
| 7 | `AttentionThreatTable` | Combat/care attention | proximity, actions, wounds, boy position, noise, care focus | threat values, target choice hints, presenter summary | `threat_actor`, `threat_target`, `delta`, `reason`, `current_target` | threat from attack, threat from boy movement, decay | wolves |
| 8 | `AggroCallFleeRadius` | Encounter AI | actor route, radius defs, threat table, flee triggers | aggro, call, flee, leash, target events | `radius_id`, `trigger`, `from`, `to`, `target`, `route_id` | aggro enter/exit, call pack, flee route, leash return | wolves |
...
| 12 | `AbilityDef` / `VerbDef` | Data/rules | data rows, tags, costs, timer, resolver table, presenter keys | executable verb contracts | `verb_id`, `ability_id`, `cost_keys`, `timer_id`, `table_id`, `presenter_key` | data validation, invalid missing table, verb execution fixture | all acts |
...
| 14 | Leash/route/respawn | Encounter/world | route graph, leash radius, reset rules, death/down state | route, return, reset, respawn/friction events | `route_id`, `node`, `leash_state`, `reset_reason`, `spawn_state` | route follow, leash break, reset, later respawn hook | wolves |
