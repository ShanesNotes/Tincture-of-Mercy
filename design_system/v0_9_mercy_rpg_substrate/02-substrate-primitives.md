...
| 13 | `ItemDef` / quality / loot hooks | Economy/data | item defs, loot tables, quality budget, source actor, source event, world context | `loot_eligibility_recorded`, `material_outcome_emitted`, later inventory hooks and reward projections | `item_id`, `quality`, `rarity`, `quantity`, `loot_table`, `source_actor`, `source_event_id`, `eligibility_reason` | wolf recoverable-body material fixture, downed non-body withholding, flee/leash withholding, seeded/scripted quality-rarity replay, eligibility trace | bread, wolves |
...
| `LootHooks_WolfRecoverableBodyEmitsMaterialOutcome` | combat reward can produce grounded material consequence through source-event-traced economy events. |
