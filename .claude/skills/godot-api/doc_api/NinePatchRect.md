...
**Props:**
- AxisStretchHorizontal: int (NinePatchRect.AxisStretchMode) = 0
- AxisStretchVertical: int (NinePatchRect.AxisStretchMode) = 0
- DrawCenter: bool = true
- MouseFilter: int (Control.MouseFilter) = 2
- PatchMarginBottom: int = 0
- PatchMarginLeft: int = 0
- PatchMarginRight: int = 0
- PatchMarginTop: int = 0
...
**Methods:**
- GetPatchMargin(int margin) -> int - Returns the size of the margin on the specified `Side`.
- SetPatchMargin(int margin, int value) - Sets the size of the margin on the specified `Side` to `value` pixels.

**Signals:**
- TextureChanged - Emitted when the node's texture changes.
