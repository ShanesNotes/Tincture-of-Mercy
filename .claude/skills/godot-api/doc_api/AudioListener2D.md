...
**Methods:**
- ClearCurrent() - Disables the AudioListener2D. If it's not set as current, this method will have no effect.
- IsCurrent() -> bool - Returns `true` if this AudioListener2D is currently active.
- MakeCurrent() - Makes the AudioListener2D active, setting it as the hearing point for the sounds. If there is already another active AudioListener2D, it will be disabled. This method will have no effect if the AudioListener2D is not added to SceneTree.
