...
**Props:**
- FallbackBaseScale: float = 1.0
- FallbackFont: Font
- FallbackFontSize: int = 16
...
**Methods:**
- GetDefaultTheme() -> Theme - Returns a reference to the default engine Theme. This theme resource is responsible for the out-of-the-box look of Control nodes and cannot be overridden.
- GetProjectTheme() -> Theme - Returns a reference to the custom project Theme. This theme resources allows to override the default engine theme for every control node in the project. To set the project theme, see `ProjectSettings.gui/theme/custom`.

**Signals:**
- FallbackChanged - Emitted when one of the fallback values had been changed. Use it to refresh the look of controls that may rely on the fallback theme items.
