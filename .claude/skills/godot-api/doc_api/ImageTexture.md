...
**Methods:**
- CreateFromImage(Image image) -> ImageTexture - Creates a new ImageTexture and initializes it by allocating and setting the data from an Image.
- SetImage(Image image) - Replaces the texture's data with a new Image. This will re-allocate new memory for the texture. If you want to update the image, but don't need to change its parameters (format, size), use `update` instead for better performance.
