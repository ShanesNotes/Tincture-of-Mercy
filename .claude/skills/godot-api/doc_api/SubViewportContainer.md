...
**Props:**
- FocusMode: int (Control.FocusMode) = 1
- MouseTarget: bool = false
- Stretch: bool = false
- StretchShrink: int = 1
...
**Methods:**
- PropagateInputEvent(InputEvent event) -> bool - Virtual method to be implemented by the user. If it returns `true`, the `event` is propagated to SubViewport children. Propagation doesn't happen if it returns `false`. If the function is not implemented, all events are propagated to SubViewports.
