...
**Methods:**
- ConnectToPeer(PacketPeerUDP packetPeer, string hostname, TLSOptions clientOptions = null) -> int - Connects a `packet_peer` beginning the DTLS handshake using the underlying PacketPeerUDP which must be connected (see `PacketPeerUDP.connect_to_host`). You can optionally specify the `client_options` to be used while verifying the TLS connections. See `TLSOptions.client` and `TLSOptions.client_unsafe`.
- DisconnectFromPeer() - Disconnects this peer, terminating the DTLS session.
