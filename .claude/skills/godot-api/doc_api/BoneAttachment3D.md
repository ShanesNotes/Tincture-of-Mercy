...
- BoneIdx: int = -1
- BoneName: string = ""
- ExternalSkeleton: NodePath
- OverridePose: bool = false
- PhysicsInterpolationMode: int (Node.PhysicsInterpolationMode) = 2
- UseExternalSkeleton: bool = false
...
**Methods:**
- GetSkeleton() -> Skeleton3D - Returns the parent or external Skeleton3D node if it exists, otherwise returns `null`.
- OnSkeletonUpdate() - A function that is called automatically when the Skeleton3D is updated. This function is where the BoneAttachment3D node updates its position so it is correctly bound when it is *not* set to override the bone pose.
