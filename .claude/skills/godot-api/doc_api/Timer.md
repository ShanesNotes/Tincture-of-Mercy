...
- OneShot: bool = false
- Paused: bool
- ProcessCallback: int (Timer.TimerProcessCallback) = 1
- TimeLeft: float
- WaitTime: float = 1.0
...
**Methods:**
- IsStopped() -> bool - Returns `true` if the timer is stopped or has not started.
- Start(float timeSec = -1) - Starts the timer, or resets the timer if it was started already. Fails if the timer is not inside the scene tree. If `time_sec` is greater than `0`, this value is used for the `wait_time`. **Note:** This method does not resume a paused timer. See `paused`.
