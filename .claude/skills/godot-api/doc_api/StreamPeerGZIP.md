...
- StartCompression(bool useDeflate = false, int bufferSize = 65535) -> int - Start the stream in compression mode with the given `buffer_size`, if `use_deflate` is `true` uses deflate instead of GZIP.
- StartDecompression(bool useDeflate = false, int bufferSize = 65535) -> int - Start the stream in decompression mode with the given `buffer_size`, if `use_deflate` is `true` uses deflate instead of GZIP.
