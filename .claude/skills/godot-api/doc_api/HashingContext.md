...
**Methods:**
- Finish() -> byte[] - Closes the current context, and return the computed hash.
- Start(int type) -> int - Starts a new hash computation of the given `type` (e.g. `HASH_SHA256` to start computation of an SHA-256).
- Update(byte[] chunk) -> int - Updates the computation with the given `chunk` of data.
