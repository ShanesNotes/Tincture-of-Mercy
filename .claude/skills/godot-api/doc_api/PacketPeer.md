...
**Props:**
- EncodeBufferMaxSize: int = 8388608
...
**Methods:**
- GetAvailablePacketCount() -> int - Returns the number of packets currently available in the ring-buffer.
- GetPacket() -> byte[] - Gets a raw packet.
- GetPacketError() -> int - Returns the error state of the last packet received (via `get_packet` and `get_var`).
- GetVar(bool allowObjects = false) -> Variant - Gets a Variant. If `allow_objects` is `true`, decoding objects is allowed. Internally, this uses the same decoding mechanism as the `@GlobalScope.bytes_to_var` method. **Warning:** Deserialized objects can contain code which gets executed. Do not use this option if the serialized object comes from untrusted sources to avoid potential security threats such as remote code execution.
- PutPacket(byte[] buffer) -> int - Sends a raw packet.
- PutVar(Variant var, bool fullObjects = false) -> int - Sends a Variant as a packet. If `full_objects` is `true`, encoding objects is allowed (and can potentially include code). Internally, this uses the same encoding mechanism as the `@GlobalScope.var_to_bytes` method.
