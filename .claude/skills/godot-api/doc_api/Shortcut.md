...
**Props:**
- Events: Godot.Collections.Array = []
...
**Methods:**
- GetAsText() -> string - Returns the shortcut's first valid InputEvent as a String.
- HasValidEvent() -> bool - Returns whether `events` contains an InputEvent which is valid.
- MatchesEvent(InputEvent event) -> bool - Returns whether any InputEvent in `events` equals `event`. This uses `InputEvent.is_match` to compare events.
