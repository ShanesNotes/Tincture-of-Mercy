...
**Props:**
- BaseScale: float = 1.0
- ColorMap: Godot.Collections.Dictionary = {}
- FixAlphaBorder: bool = false
- PremultAlpha: bool = false
...
**Methods:**
- CreateFromString(string source, float scale = 1.0, float saturation = 1.0, Godot.Collections.Dictionary colorMap = {}) -> DPITexture - Creates a new DPITexture and initializes it by allocating and setting the SVG data to `source`.
- GetScaledRid() -> Rid - Returns the RID of the texture rasterized to match the oversampling of the currently drawn canvas item.
- GetSource() -> string - Returns this SVG texture's source code.
- SetSizeOverride(Vector2i size) - Resizes the texture to the specified dimensions.
- SetSource(string source) - Sets this SVG texture's source code.
