...
**Methods:**
- Load(string path) -> int - Loads a certificate from `path` ("*.crt" file).
- LoadFromString(string string) -> int - Loads a certificate from the given `string`.
- Save(string path) -> int - Saves a certificate to the given `path` (should be a "*.crt" file).
- SaveToString() -> string - Returns a string representation of the certificate, or an empty string if the certificate is invalid.
