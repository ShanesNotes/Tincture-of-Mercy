...
**Props:**
- Bias: float = 1.5
- DynamicRange: float = 2.0
- Energy: float = 1.0
- Interior: bool = false
- NormalBias: float = 0.0
- Propagation: float = 0.5
- UseTwoBounces: bool = true
...
**Methods:**
- Allocate(Transform3D toCellXform, Aabb aabb, Vector3 octreeSize, byte[] octreeCells, byte[] dataCells, byte[] distanceField, int[] levelCounts) - Initializes this VoxelGIData with the specified data. `octree_cells` must be a multiple of 32. `octree_cells` must be double the size of `data_cells`. The allocated data can be retrieved later using the various getter methods.
- GetBounds() -> Aabb - Returns the bounds of the baked voxel data as an AABB, which should match `VoxelGI.size` after being baked (which only contains the size as a Vector3). **Note:** If the size was modified without baking the VoxelGI data, then the value of `get_bounds` and `VoxelGI.size` will not match.
- GetDataCells() -> byte[] - Returns the baked cell data for this VoxelGIData.
- GetLevelCounts() -> int[] - Returns the baked level counts for this VoxelGIData.
- GetOctreeCells() -> byte[] - Returns the baked octree cell data for this VoxelGIData.
- GetOctreeSize() -> Vector3 - Returns the baked octree size for this VoxelGIData, which corresponds to the number of subdivisions per axis. This can be viewed in the editor by hovering the **Bake VoxelGI** button at the top of the 3D editor viewport when a VoxelGI node is selected and looking at the **Subdivisions** field in the tooltip.
- GetToCellXform() -> Transform3D - Returns the baked cell transform for this VoxelGIData.
