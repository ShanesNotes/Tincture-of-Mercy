...
- DrawOrder: int (CPUParticles3D.DrawOrder) = 0
- EmissionBoxExtents: Vector3
- EmissionColors: Color[] = PackedColorArray()
- EmissionNormals: Vector3[]
- EmissionPoints: Vector3[]
...
- EmissionShape: int (CPUParticles3D.EmissionShape) = 0
...
- ConvertFromParticles(Node particles) - Sets this node's properties to match a given GPUParticles3D node with an assigned ParticleProcessMaterial.
- GetParamCurve(int param) -> Curve - Returns the Curve of the parameter specified by `Parameter`.
...
- GetParticleFlag(int particleFlag) -> bool - Returns the enabled state of the given particle flag.
...
- Restart(bool keepSeed = false) - Restarts the particle emitter. If `keep_seed` is `true`, the current random seed will be preserved. Useful for seeking and playback.
- SetParamCurve(int param, Curve curve) - Sets the Curve of the parameter specified by `Parameter`. Should be a unit Curve.
- SetParamMax(int param, float value) - Sets the maximum value for the given parameter.
- SetParamMin(int param, float value) - Sets the minimum value for the given parameter.
- SetParticleFlag(int particleFlag, bool enable) - Enables or disables the given particle flag.
