...
**Methods:**
- Process(const void* srcBuffer, AudioFrame* rDstBuffer, int frameCount) - Called by the AudioServer to process this effect. When `_process_silence` is not overridden or it returns `false`, this method is called only when the bus is active. **Note:** It is not useful to override this method in GDScript or C#. Only GDExtension can take advantage of it.
- ProcessSilence() -> bool - Override this method to customize the processing behavior of this effect instance. Should return `true` to force the AudioServer to always call `_process`, even if the bus has been muted or cannot otherwise be heard.
