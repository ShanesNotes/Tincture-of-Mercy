...
- Clamp(Vector3i min, Vector3i max) -> Vector3i - Returns a new vector with all components clamped between the components of `min` and `max`, by running `@GlobalScope.clamp` on each component.
- Clampi(int min, int max) -> Vector3i - Returns a new vector with all components clamped between `min` and `max`, by running `@GlobalScope.clamp` on each component.
- DistanceSquaredTo(Vector3i to) -> int - Returns the squared between this vector and `to`. This method runs faster than `distance_to`, so prefer it if you need to compare vectors or need the squared distance for some formula.
- DistanceTo(Vector3i to) -> float - Returns the between this vector and `to`.
...
- Max(Vector3i with) -> Vector3i - Returns the component-wise maximum of this and `with`, equivalent to `Vector3i(maxi(x, with.x), maxi(y, with.y), maxi(z, with.z))`.
...
- Maxi(int with) -> Vector3i - Returns the component-wise maximum of this and `with`, equivalent to `Vector3i(maxi(x, with), maxi(y, with), maxi(z, with))`.
- Min(Vector3i with) -> Vector3i - Returns the component-wise minimum of this and `with`, equivalent to `Vector3i(mini(x, with.x), mini(y, with.y), mini(z, with.z))`.
...
- Mini(int with) -> Vector3i - Returns the component-wise minimum of this and `with`, equivalent to `Vector3i(mini(x, with), mini(y, with), mini(z, with))`.
...
- Snapped(Vector3i step) -> Vector3i - Returns a new vector with each component snapped to the closest multiple of the corresponding component in `step`.
- Snappedi(int step) -> Vector3i - Returns a new vector with each component snapped to the closest multiple of `step`.
