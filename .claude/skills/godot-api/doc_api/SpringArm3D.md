...
**Methods:**
- AddExcludedObject(Rid rID) - Adds the PhysicsBody3D object with the given RID to the list of PhysicsBody3D objects excluded from the collision check.
- ClearExcludedObjects() - Clears the list of PhysicsBody3D objects excluded from the collision check.
- GetHitLength() -> float - Returns the spring arm's current length.
- RemoveExcludedObject(Rid rID) -> bool - Removes the given RID from the list of PhysicsBody3D objects excluded from the collision check.
