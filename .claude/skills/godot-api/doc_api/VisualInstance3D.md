...
**Props:**
- Layers: int = 1
- SortingOffset: float = 0.0
- SortingUseAabbCenter: bool
...
**Methods:**
- GetAabb() -> Aabb
- GetAabb() -> Aabb - Returns the AABB (also known as the bounding box) for this VisualInstance3D.
- GetBase() -> Rid - Returns the RID of the resource associated with this VisualInstance3D. For example, if the Node is a MeshInstance3D, this will return the RID of the associated Mesh.
- GetInstance() -> Rid - Returns the RID of this instance. This RID is the same as the RID returned by `RenderingServer.instance_create`. This RID is needed if you want to call RenderingServer functions directly on this VisualInstance3D.
- GetLayerMaskValue(int layerNumber) -> bool - Returns whether or not the specified layer of the `layers` is enabled, given a `layer_number` between 1 and 20.
- SetBase(Rid base) - Sets the resource that is instantiated by this VisualInstance3D, which changes how the engine handles the VisualInstance3D under the hood. Equivalent to `RenderingServer.instance_set_base`.
- SetLayerMaskValue(int layerNumber, bool value) - Based on `value`, enables or disables the specified layer in the `layers`, given a `layer_number` between 1 and 20.
