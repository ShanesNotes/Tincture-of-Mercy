...
- RegionFilterClipEnabled: bool = false
...
**Methods:**
- GetRect() -> Rect2 - Returns a Rect2 representing the Sprite2D's boundary in local coordinates. **Example:** Detect if the Sprite2D was clicked:
- IsPixelOpaque(Vector2 pos) -> bool - Returns `true` if the pixel at the given position is opaque, `false` otherwise. Also returns `false` if the given position is out of bounds or this sprite's `texture` is `null`. `pos` is in local coordinates.
