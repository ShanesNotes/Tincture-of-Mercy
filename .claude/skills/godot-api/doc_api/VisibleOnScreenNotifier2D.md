...
**Methods:**
- IsOnScreen() -> bool - If `true`, the bounding rectangle is on the screen. **Note:** It takes one frame for the VisibleOnScreenNotifier2D's visibility to be determined once added to the scene tree, so this method will always return `false` right after it is instantiated, before the draw pass.

**Signals:**
- ScreenEntered - Emitted when the VisibleOnScreenNotifier2D enters the screen.
- ScreenExited - Emitted when the VisibleOnScreenNotifier2D exits the screen.
