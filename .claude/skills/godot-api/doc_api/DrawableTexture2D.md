...
**Methods:**
- BlitRect(Rect2i rect, Texture2D source, Color modulate = Color(1, 1, 1, 1), int mipmap = 0, Material material = null) - Draws to given `rect` on this texture by copying from the given `source`. A `modulate` color can be passed in for the shader to use, but defaults to White. The `mipmap` value can specify a draw to a lower mipmap level. The `material` parameter can take a ShaderMaterial with a TextureBlit Shader for custom drawing behavior.
- BlitRectMulti(Rect2i rect, Texture2D[] sources, DrawableTexture2D[] extraTargets, Color modulate = Color(1, 1, 1, 1), int mipmap = 0, Material material = null) - Draws to the given `rect` on this texture, as well as on up to 3 DrawableTexture `extra_targets`. All `extra_targets` must be the same size and DrawableFormat as the original target, otherwise the Shader may fail. Expects up to 4 Texture `sources`, but will replace missing `sources` with default Black Textures.
- GenerateMipmaps() - Re-calculates the mipmaps for this texture on demand.
- GetUseMipmaps() -> bool - Returns `true` if mipmaps are set to be used on this DrawableTexture.
- SetFormat(int format) - Sets the format of this DrawableTexture.
- SetUseMipmaps(bool mipmaps) - Sets if mipmaps should be used on this DrawableTexture.
- Setup(int width, int height, int format, Color color = Color(1, 1, 1, 1), bool useMipmaps = false) - Initializes the DrawableTexture to a White texture of the given `width`, `height`, and `format`.
