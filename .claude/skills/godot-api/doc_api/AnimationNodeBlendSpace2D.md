...
**Props:**
- AutoTriangles: bool = true
- BlendMode: int (AnimationNodeBlendSpace2D.BlendMode) = 0
- CyclicLength: float = 0.0
- MaxSpace: Vector2 = Vector2(1, 1)
- MinSpace: Vector2 = Vector2(-1, -1)
- Snap: Vector2 = Vector2(0.1, 0.1)
- Sync: bool
- SyncMode: int (AnimationNodeBlendSpace2D.SyncMode) = 0
- XLabel: string = "x"
- YLabel: string = "y"
...
**Methods:**
- AddBlendPoint(AnimationRootNode node, Vector2 pos, int atIndex = -1, StringName name = &"") - Adds a new point with `name` that represents a `node` at the position set by `pos`. You can insert it at a specific index using the `at_index` argument. If you use the default value for `at_index`, the point is inserted at the end of the blend points array. **Note:** If no name is provided, safe index is used as reference. In the future, empty names will be deprecated, so explicitly passing a name is recommended.
- AddTriangle(int x, int y, int z, int atIndex = -1) - Creates a new triangle using three points `x`, `y`, and `z`. Triangles can overlap. You can insert the triangle at a specific index using the `at_index` argument. If you use the default value for `at_index`, the point is inserted at the end of the blend points array.
- FindBlendPointByName(StringName name) -> int - Returns the index of the blend point with the given `name`. Returns `-1` if no blend point with that name is found.
- GetBlendPointCount() -> int - Returns the number of points in the blend space.
- GetBlendPointName(int point) -> StringName - Returns the name of the blend point at index `point`.
- GetBlendPointNode(int point) -> AnimationRootNode - Returns the AnimationRootNode referenced by the point at index `point`.
- GetBlendPointPosition(int point) -> Vector2 - Returns the position of the point at index `point`.
- GetTriangleCount() -> int - Returns the number of triangles in the blend space.
- GetTrianglePoint(int triangle, int point) -> int - Returns the position of the point at index `point` in the triangle of index `triangle`.
- RemoveBlendPoint(int point) - Removes the point at index `point` from the blend space.
- RemoveTriangle(int triangle) - Removes the triangle at index `triangle` from the blend space.
- ReorderBlendPoint(int fromIndex, int toIndex) - Swaps the blend points at indices `from_index` and `to_index`, exchanging their positions and properties.
- SetBlendPointName(int point, StringName name) - Sets the name of the blend point at index `point`. If the name conflicts with an existing point, a unique name will be generated automatically.
- SetBlendPointNode(int point, AnimationRootNode node) - Changes the AnimationNode referenced by the point at index `point`.
- SetBlendPointPosition(int point, Vector2 pos) - Updates the position of the point at index `point` in the blend space.

**Signals:**
- TrianglesUpdated - Emitted every time the blend space's triangles are created, removed, or when one of their vertices changes position.
