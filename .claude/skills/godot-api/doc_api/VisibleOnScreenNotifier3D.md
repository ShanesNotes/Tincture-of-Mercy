...
**Methods:**
- IsOnScreen() -> bool - Returns `true` if the bounding box is on the screen. **Note:** It takes one frame for the VisibleOnScreenNotifier3D's visibility to be assessed once added to the scene tree, so this method will always return `false` right after it is instantiated.

**Signals:**
- ScreenEntered - Emitted when the VisibleOnScreenNotifier3D enters the screen.
- ScreenExited - Emitted when the VisibleOnScreenNotifier3D exits the screen.
