...
**Methods:**
- Post(int count = 1) - Lowers the Semaphore, allowing one thread in, or more if `count` is specified.
- TryWait() -> bool - Like `wait`, but won't block, so if the value is zero, fails immediately and returns `false`. If non-zero, it returns `true` to report success.
