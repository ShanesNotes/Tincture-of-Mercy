...
**Methods:**
- IsPositionEnabled() -> bool - Returns `true` if `enable` has `TRANSFORM_FLAG_POSITION`.
- IsRotationEnabled() -> bool - Returns `true` if `enable` has `TRANSFORM_FLAG_ROTATION`.
- IsScaleEnabled() -> bool - Returns `true` if `enable` has `TRANSFORM_FLAG_SCALE`.
- SetPositionEnabled(bool enabled) - Sets `TRANSFORM_FLAG_POSITION` into `enable`.
- SetRotationEnabled(bool enabled) - Sets `TRANSFORM_FLAG_ROTATION` into `enable`.
- SetScaleEnabled(bool enabled) - Sets `TRANSFORM_FLAG_SCALE` into `enable`.
